...
API Endpoint的地址请见[这里](endpoint.md)

# Broker 合约公共端点

## `brokerInfo`

获取BtcFX的交易规则和合约symbol的信息（精度单位等信息），包括合约的风险限额和乘数等信息。
...
名称|类型|是否强制|默认|描述
------------ | ------------ | ------------ | ------------ | ----
`type`|string|`NO`|| 交易类型，支持的类型现为`token`（币币）、`contracts`（合约）。如果没有发送此参数，所有交易类型的symbol信息都会被返回。
